...
export default function CoinTile({ symbol, value, profitLoss, iconUrl }: CoinTileProps) {
  ...
  return (
    ...
    >
      {iconUrl && (
        <img src={iconUrl} alt={symbol} className="w-8 h-8 mb-2" />
      )}
      <h4 className="text-lg font-semibold mb-1">{symbol}</h4>
      <p className="text-sm text-gray-500 mb-1">{formatCurrency(value)}</p>
      <p
        className={`text-sm font-bold ${profitLoss >= 0 ? "text-green-600" : "text-red-500"}`}
      >
