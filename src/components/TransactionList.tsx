...
export default function TransactionList() {
  ...
  const [sortBy, setSortBy] = useState<
    "timestamp" | "coin_name" | "amount" | "total_value"
  >("timestamp");
  ...
  const sortedTransactions = [...selectedPortfolio.transactions].sort(
    (a, b) => {
      const get = (field: typeof sortBy) => {
        if (field === "timestamp")
          return (t: Transaction) => new Date(t.timestamp).getTime();
        if (field === "coin_name")
          return (t: Transaction) => t.coin_name.toLowerCase();
